...
DWORD WINAPI miningThreadProc(LPVOID)
{
    ...
    while (!state)
    {
        ...
        else
        {
            ...
            memset(neuronValues, 0xFF, sizeof(neuronValues[0]));
    ...
    else
    {
        ...
        while (!state)
        {
            ...
            if (delta >= 1000)
            {
                ...
                KangarooTwelve(minerPublicKey, 32, idBytesChecksum, sizeof(idBytesChecksum));
                for (int i = 0; i < sizeof(idBytesChecksum); i++)
                {
